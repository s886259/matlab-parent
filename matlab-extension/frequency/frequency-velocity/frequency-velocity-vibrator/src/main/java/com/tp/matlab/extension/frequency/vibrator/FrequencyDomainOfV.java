...
public class FrequencyDomainOfV {
    ...
    public Map<String, Object> execute(@NonNull final List<Double> a) throws JsonProcessingException {
        ...
        final DoubleMax pm_max = MatlabUtils.getMax(vi);
        //mf=f(m);    %峰值对应频率值
        ...
        final List<Double> valu_xiebo = new Screen(num_f, vi).execute();
        ...
        final List<Double> valu_biandai = new Screen(num_f1, vi).execute();
        ...
        final List<Double> dB = f1.stream()
                .map(i -> 20 * Math.log(vi.get(i - 1) / vi.get(500 - 1)))
                .collect(toList());
        //to result
        final List<BigDecimal> x = spectrumResult.getF().stream()
                .map(NumberFormatUtils::roundToDecimal)
                .collect(toList());
        final List<BigDecimal> y = spectrumResult.getVi().stream()
                .map(NumberFormatUtils::roundToDecimal)
                .collect(toList());
