...
//			Color.CYAN, Color.BLUE);
	...
	private PamSymbol navAidSymbol = new PamSymbol(PamSymbol.SYMBOL_CIRCLE, 10, 10, true, Color.yellow, Color.yellow);
...
//		setPamSymbol(aisVessel);
	...
	protected Rectangle drawRangeOnRadar(Graphics g, PamDataUnit pamDataUnit, GeneralProjector generalProjector) {
		...
		getPamSymbol(); // make sure that the symbol exists. 
...
//					positionReport = pr.get(i);
	...
	private PamSymbol getNavaidSymbol(int staticType) {
		PamSymbol mainSymbol = getPamSymbol();
	...
	public PamSymbol getPamSymbol() {
		...
		if (aisVessel == null) {
			setPamSymbol(new Vessel(Color.BLUE));
		}
		return aisVessel;
	}
