...
//	private double[] niRanges = {.1, .2, .5, 1, 2, 10};//{10, 5, 2.5, 1, 0.5, 0.25, 0.1, 0.05};
	
	private JCheckBox allowMultiBoard;
	
	private NIDaqParams niParameters = new NIDaqParams();
...
//	Vector<RawDataUnit> newDataUnits;
...
//	TargetDataLine targetDataLine;
	...
	private List<RawDataUnit> newDataUnits;
	...
	public NIDAQProcess (AcquisitionControl daqControl) {
		...
		setSettingsUnitName();
		nidaq = new Nidaq(this);
		niDevices = nidaq.getDevicesList();
		PamSettingManager.getInstance().registerSettings(this);
		niChannelListPanel = new NIChannelListPanel(this);
...
//		dataUnitSamples = Math.max(dataUnitSamples, 1000);
		...
		newDataUnits = daqControl.getAcquisitionProcess().getNewDataUnits();
...
//	}
	...
	public String getSystemType() {
		return "National Instruments DAQ Cards";
	}
...
//		boolean b = allowMultiBoard.isSelected();
		niChannelListPanel.enableMultiBoardOps();
	}
	...
	private String settingsUnitName = "NI-DAQ Card System";
	...
	class SelectBoard implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			if (allowMultiBoard.isSelected() == false) {
				niChannelListPanel.setAllBoards(audioDevices.getSelectedIndex());
			}
			checkDevice();
		}
	}
...
//		}
	...
	/* (non-Javadoc)
	 * @see Acquisition.DaqSystem#fillXMLParameters(org.w3c.dom.Document, org.w3c.dom.Element)
	 */
	@Override
	public boolean fillXMLParameters(Document doc, Element paramsEl) {
		int[] deviceList = niParameters.getDeviceList();
		int[] channelList = niParameters.getHwChannelList();
		double[] gains = niParameters.getAIRange(0);
		int nChans = daqControl.acquisitionParameters.nChannels;
		int iDev;
		String devName;
		String devType;
		paramsEl.setAttribute("terminalConfiguration", new Integer(niParameters.terminalConfiguration).toString());
		for (int i = 0; i < nChans; i++) {
			Element el = doc.createElement("Chan"+i);
			iDev = deviceList[i];
			devName = nidaq.getDeviceName(iDev+1);
			el.setAttribute("DeviceName", devName);
//			devType = nidaq.getDeviceType(iDev+1);
			devType = nidaq.getDeviceName(iDev+1);
			el.setAttribute("DeviceType", devType);
			el.setAttribute("HardwareChannel", new Integer(channelList[i]).toString());
			gains = niParameters.getAIRange(i);
			el.setAttribute("RangeLo", new Double(gains[0]).toString());
			el.setAttribute("RangeHi", new Double(gains[1]).toString());
