...
public class Nidaq {
	...
	static private NIDAQProcess niDaqProcess;

	public Nidaq(NIDAQProcess niDaqProcess) {
		super();
		if (niDaqProcess != null) {
	...
	public int javaPreparePlayback(int boardNumber, int sampleRate,
			int bufferSamples, int[] outputChannelList) {
		if (loadLibraryOK) {
			return jniPreparePlayback(boardNumber, sampleRate, bufferSamples, outputChannelList);
		}
		else {
			return 0;
		}
	...
	public ArrayList<NIDeviceInfo> getDevicesList() {
		ArrayList<NIDeviceInfo> devices = new ArrayList<NIDeviceInfo>();
		...
		for (int i = 0; i <= nDevices; i++) {
			...
			ranges = getAIVoltageRanges(i);
			if (devName != null) {
				devices.add(devInfo = new NIDeviceInfo(i, devName, devType, serialNum, isSimulated, 
						isSimultaneous, inputChannels, outputChannels, ranges));
...
//						+ System.getProperty ("java.library.path"));
	...
	public static void fullBuffer(int iChan, double[] data) {
		...
		//				data.length, iChan));
		niDaqProcess.fullBuffer(iChan, data);
	}
