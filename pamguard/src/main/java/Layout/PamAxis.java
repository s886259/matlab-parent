...
//	private String longestString;
	...
	public void drawAxis(Graphics g) {
		...
		if (interval == INTERVAL_AUTO) {
			theInterval = getInterval((maxVal - minVal), (int) totalPixs, 80);
		}
		if (drawLine)
...
//			DrawTickAndLabel(g2d, x, y, Math.pow(10, axisValue));
	...
	private void drawTickAndLabel(Graphics2D g2d, double x, double y,
		...
		String str;
		int labelWidth;
		g2d.drawLine((int) x + tickLengthX[0], (int) y + tickLengthY[0],
				xt = (int) x + tickLengthX[1], yt = (int) y + tickLengthY[1]);
		// print the label
	...
	public static double getDefaultScaleEnd(double val, double minVal) {
		int sign = 1;
		if (val < 0) {
			val = -val;
			sign = -1;
		...
		scaleEnd = Math.max(scaleEnd, Math.abs(minVal));
