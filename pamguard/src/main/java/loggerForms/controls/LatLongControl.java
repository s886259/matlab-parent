 ...
 * will probably extend this to give GPSTimestamp which will bundle in a timestamp field too.
...
//	private int[] channels;
	...
	public LatLongControl(ControlDescription controlDescription,
		...
		if (
				LatLong.getFormatStyle()==LatLong.FORMAT_DECIMALMINUTES
				){
			latTemplate=String.format("##%s##%s###%s",deg,pt,min);
		} else if(
				LatLong.getFormatStyle()==LatLong.FORMAT_MINUTESSECONDS
				){
...
//			System.out.println("but: "+e.getButton());
	...
	private void makeComponent() {
		...
		double charwid = textFieldLat.getFontMetrics(textFieldLat.getFont()).getMaxAdvance()/2;
	...
	void inputLatLon(){
		...
		if (defLatLong==null){
			...
			if(channelMap==null||channelMap<1){
				defLatLong=ArrayManager.getArrayManager().getCurrentArray().getHydrophoneLocator().getReferenceLatLong(timeMilliseconds);
			}else{
...
//					getData(LatLong.getDataFlavor());
//			System.out.println("CBLLdata:"+data);
//			dataVal= (LatLong)data;
//			data2.getTransferDataFlavors();
			try {
...
//				System.out.println("CBStdataCt:"+data);
//				dataVal= new LatLong((String)data);
//				System.out.println("CBStdatVCt:"+dataVal);
...
//		return PamCalendar.getTimeStamp(PamCalendar.msFromDateString));
	...
	public int autoUpdate() {
		...
		if(channelMap==null||channelMap==0){
			...
			setData(ArrayManager.getArrayManager().getCurrentArray().getHydrophoneLocator().getReferenceLatLong(timeMilliseconds));
		}else{
...
//		other options
//		lat==Double.NaN||lat==Double.POSITIVE_INFINITY||lat==Double.NEGATIVE_INFINITY
		...
		if (lon==null&lat!=null){
			if (dataError==null)dataError="";
			dataError+="\n"+ controlDescription.getTitle()+" -Longitude is incorrect";
		}
		...
		if (req==true){
			if (data==null){
				if (dataError==null)dataError="";
				dataError+="\n"+ controlDescription.getTitle()+" is a required field.";
			}
...
//		return dataError;
	...
	public int fillNMEAControlData(NMEADataUnit dataUnit) {
		String latStr = NMEADataBlock.getSubString(dataUnit.getCharData(),
				controlDescription.getNmeaPosition());
		String lonStr = NMEADataBlock.getSubString(dataUnit.getCharData(),
