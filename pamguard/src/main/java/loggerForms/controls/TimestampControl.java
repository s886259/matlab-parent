...
public class TimestampControl extends LoggerControl {
	...
	private void makeComponent() {
		...
		double charwid = textFieldDate.getFontMetrics(textFieldDate.getFont()).getMaxAdvance()/2;
...
//		System.out.println("tstp: "+PamCalendar.getTimeStamp(PamCalendar.msFromDateString(date+" "+time)));
		...
		return PamCalendar.getTimeStamp(PamCalendar.msFromDateString(date+" "+time));
...
//		setData(PamCalendar.getTimeInMillis());
	 ...
	 */
	@Override
	public void setData(Object timeInMillis) {//long
...
//			textFieldTime.setValue(null);
		...
		if (timeInMillis.getClass().isAssignableFrom(Timestamp.class)){
			timeInMillis=PamCalendar.millisFromTimeStamp((Timestamp) timeInMillis);
//			System.out.println("setData (timestamp) rather than long");
		}
		
		if ((Long) timeInMillis==null){
