...
public class LoggerFormGraphics implements PanelOverlayDraw {
	...
	private Rectangle plotLatLongControls(Graphics g, FormsDataUnit dataUnit,
		...
		ArrayList<ControlDescription> controlDescriptions = formDescription.getInputControlDescriptions();
		...
		for (int i = 0; i < nControls; i++) {
			...
			if (data[i].getClass() == LatLong.class) {
	...
	private void drawHeadingArrow(Graphics g, GeneralProjector projector, Point start, HEADINGinfo headingInfo, Double heading) {
		...
		PamSymbol.drawArrow(g, (int)start.x, (int)start.y, endX, endY, 5);
	...
	private PamSymbol getPlotSymbol(FormsDataUnit dataUnit) {
		...
		for (int i = 0; i < nControls; i++) {
			...
			if (controlDescription.getEType() == ControlTypes.LOOKUP) {
				...
				if (formPlotOptions.isPlotControl(i+1, lutIndex+1) || formPlotOptions.isPlotControl(i+1, 0)) {
					lutSymbol = lutList.getLookupItem(lutIndex).getSymbol();
					if (lutSymbol != null) {
						return lutSymbol;
					}
				}
	...
	private void createSymbols() {
		int symbolType = PamSymbol.SYMBOL_CIRCLE;
		int symbolSize = 6;
		Color fillCol = Color.BLUE;
		Color lineCol = Color.BLUE;
		boolean fill = false;
		/*
		 * Now see if the form description has got any more specific information
		 * about any of the above.
		 */
		PropertyDescription formProperty = formDescription.findProperty(propertyTypes.BLOBSIZE);
		if (formProperty != null) {
			Integer length = formProperty.getLength();
			if (length != null) {
				symbolSize = length;
			}
			String colString = formProperty.getColour();
			if (colString != null) {
				Color c = PamColors.interpretColourString(colString);
				if (c != null) {
					fillCol = lineCol = c;
				}
			}
		...
		formProperty = formDescription.findProperty(propertyTypes.SYMBOLTYPE);
		if (formProperty != null) {
			symbolType = PamSymbol.interpretTextCode(formProperty.getTitle());
			Integer symbolLen = formProperty.getLength();
			if (symbolLen != null) {
				symbolSize = symbolLen;
			}
			Boolean fillProp = formProperty.getPlot();
			if (fillProp != null) {
				fill = fillProp;
			}
			String colString = formProperty.getColour();
		...
		formProperty = formDescription.findProperty(propertyTypes.PLOT);
		if (formProperty != null) {
			Boolean fillProp = formProperty.getPlot();
			if (fillProp != null) {
				fill = fillProp;
		...
		standardSymbol = new PamSymbol(symbolType, symbolSize, symbolSize, fill, fillCol, lineCol);
	}
