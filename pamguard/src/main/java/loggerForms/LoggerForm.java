...
import staticLocaliser.panels.OptionsDialog;

import classifier.Classifier.ProbabilityType;
...
public class LoggerForm{
	...
	JButton saveButton = new PamButton("Save");
	JButton clearButton = new PamButton("Clear");
	JButton cancelButton = new PamButton("Cancel");

	PamLabel ctrlHint, ctrlMesssage, lastSaved ;//, formMessage;
	...
	private void setupNMEAReading(PropertyDescription nmeaProperty) {
		...
		if (nmeaUnit == null) {
			OptionsDialog.showWarning(null, getFormDescription().getFormNiceName(), 
	...
	private class NMEAMonitor implements PamObserver {
	...
	private class GPSObserver implements PamObserver {
	...
	private void fillCentrePanel() {
		...
		for (ControlDescription c:controlDescriptions){
			...
			if(c.isInput()){
				...
				if (currentControl == null) {
					boolean tst = c.isInput();
				}
				inputControls.add(currentControl);
				currentRow.add(currentControl.getComponent());
			}else if(c.getEType()==ControlDescription.ControlTypes.NEWLINE){
		...
		boolean isNormal;
		if (isSubTabs==false&&isSubForm==false&&isPopup==false){
			isNormal=true;
		}else{
			isNormal=false;
		}
		...
		if (isSubTabs||isSubForm||isPopup){
			if (!noCancel){
	 ...
	 */
	public Object[] transferControlDataToArray() {
	...
	private void save() {
		 ...
		 */
		Object[] formData = transferControlDataToArray();
		...
		if (NewOrEdit==NewDataForm){
			...
			//mapController.mapProcess.getMapCommentDataBlock().addPamData(mapComment);
		}
