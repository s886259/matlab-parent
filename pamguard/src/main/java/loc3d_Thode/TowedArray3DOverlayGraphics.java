...
public class TowedArray3DOverlayGraphics extends PamDetectionOverlayGraphics {
	...
	public TowedArray3DOverlayGraphics(TowedArray3DProcess towedArray3DProcess) {
		...
		super(towedArray3DProcess.localizationDataBlock);

		this.towedArray3DProcess = towedArray3DProcess;

		//setPamSymbol(new PamSymbol(PamSymbol.SYMBOL_CIRCLE, 16, 16, true, Color.RED, Color.GREEN));
		circle_size=16;
		/*
		 * It may have picked up a symbol from the new PamSymbolManager in which case we don't want
		 * to re-set it, so only crate if one doesn't exist. Doug. 23 Jan, 2008
		 */
		if (getPamSymbol() == null) {
			setPamSymbol(new PamSymbol(PamSymbol.SYMBOL_CIRCLE, 16, 16, true, Color.RED, Color.GREEN));
		}
		getPamSymbol().setHeight(circle_size);
		getPamSymbol().setWidth(circle_size);
	}
...
//		LatLong centreLatLong = getDetectionCentre(pamDetection);
		...
		if (localisation == null || 
			...
			return getPamSymbol(detection3D).draw(g, detectionCentre.getXYPoint());
		}
