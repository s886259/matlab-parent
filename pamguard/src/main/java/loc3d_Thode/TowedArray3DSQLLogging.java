...
//		dateItem.setValue(PamCalendar.getTimeStamp(ldu.getTimeMilliseconds()));
		...
		depthItem.setValue(ldu.getDepth(0));
	...
	protected PamDataUnit createDataUnit(long timeMilliseconds, int databaseIndex) {
		...
		if (!Double.isNaN(v)) {
			...
			ldu.addLocContents(AbstractLocalisation.HAS_RANGE);
		}
		...
		if (!Double.isNaN(v)) {
			...
			ldu.addLocContents(AbstractLocalisation.HAS_BEARING);
		}
		tadu.setDepth(v = (Double) depthItem.getValue());
//		System.out.println("DEpth = " + v);
		if (!Double.isNaN(v)) {
			tadu.setHasDepth(true);
			ldu.addLocContents(AbstractLocalisation.HAS_DEPTH);
		}
