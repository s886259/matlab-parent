...
//		groupDetDataBlock.setOverlayDraw(new FindAnchorOverlayGraphics(towedArray3DController));
		 ...
		 */ 
		localizationDataBlock.setLocalisationContents(AbstractLocalisation.HAS_BEARING | 
				AbstractLocalisation.HAS_RANGE | AbstractLocalisation.HAS_DEPTH | AbstractLocalisation.HAS_AMBIGUITY);
		localizationDataBlock.addLocalisationContents(AbstractLocalisation.HAS_LATLONG);
...
//		localizationDataBlock.SetLogging(new PamDetectionLogging(localizationDataBlock, SQLLogging.UPDATE_POLICY_OVERWRITE));
	...
	public void pamStart() {
		...
		int hydrophoneList = ((AcquisitionProcess) this.getSourceProcess()).
		getAcquisitionControl().ChannelsToHydrophones(usedChannels);
		...
		subarray_deck_distance = Math.abs(array.getHydrophone(0).getCoordinate(1));
		//this.tdd_maxtime=ArrayManager.getArrayManager().getCurrentArray().getSeparationInMillis(hydrophoneList);

		crossStationMatcher= new CrossStationMatcher(1,this.tdd_maxtime,towedArray3DController.towedArray3DProcessParameters.percentICIMatchError/100.0);
			...
			double eta, float tilt0[], TowedArray3DDataUnit ldu)
	{
		...
		if (range_r<10E6){
			ldu.getLocalisation().setLocContents(AbstractLocalisation.HAS_BEARING | AbstractLocalisation.HAS_RANGE|AbstractLocalisation.HAS_DEPTH);
			ldu.getLocalisation().addLocContents(AbstractLocalisation.HAS_LATLONG);
			ldu.setLatLong();
		}
		else
			ldu.getLocalisation().setLocContents(0);
			...
			double bearing_r, TowedArray3DDataUnit ldu)
	{
		...
		ldu.getLocalisation().setLocContents(AbstractLocalisation.HAS_BEARING | AbstractLocalisation.HAS_RANGE);
		ldu.getLocalisation().addLocContents(AbstractLocalisation.HAS_LATLONG);
		ldu.setLatLong();
	...
	public void prepareProcess() {
		...
		for (int i = 0; i < 2; i++) {
			//if (((1<<i) & usedChannels) > 0) {
			stationDetectors[i] = new StationQueue(i);
