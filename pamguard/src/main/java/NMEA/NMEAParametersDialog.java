...
public class NMEAParametersDialog extends PamDialog implements ActionListener {
	...
	private JComboBox bitsPerSecondComboBox = new JComboBox();	
	...
	void SetParams(NMEAParameters nmeaParameters){
		...
		for(int i = 0; i<commPortIds.size(); i++){
			...
			if(commPortIds.get(i).getName().equals(nmeaParameters.serialPortName)){
...
//	 This method returns the selected radio button in a button group
	...
	private void populateComboBoxes(){
		...
		for(int i = 0; i<portIds.size(); i++){
			portComboBox.addItem(portIds.get(i).getName());
		}
