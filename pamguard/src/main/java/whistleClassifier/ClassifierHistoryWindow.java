 ...
 *
 */
public class ClassifierHistoryWindow implements PamObserver {
	...
	private void createSymbols(String[] speciesList) {
		...
		for (int i = 0; i < speciesList.length; i++) {
			...
			speciesSymbols[i] = new PamSymbol(PamSymbol.SYMBOL_DIAMOND, symbolSize, symbolSize, true, col, col);
	...
	private String[] getSpeciesList() {
		if (whistleClassifierControl.getWhistleClassificationParameters().fragmentClassifierParams != null) {
			return whistleClassifierControl.getWhistleClassificationParameters().fragmentClassifierParams.speciesList;
		}
		return null;
	}
	...
	public void update(PamObservable o, PamDataUnit arg) {
		...
		//		WhistleClassificationDataUnit wcdu = (WhistleClassificationDataUnit) arg;

		//		double[] likelyhoods = wcdu.getSpeciesLikelyhoods();
	...
	class HistoryWindowBorder extends PamBorderPanel {
		...
		public HistoryWindowBorder() {
			...
			if (isViewer) {
				pamScroller = new PamScroller(getObserverName(), AbstractPamScroller.HORIZONTAL, 
						1000, 3600000, true);
...
//			repaintBoth();
			pamScroller.setVisibleAmount((long) (newValue*1000.));
	...
	class HistoryPlotPanel extends JPanelWithPamKey {
		...
		public void paintForRun(Graphics g) {
			...
			synchronized (whistleClasificationDataBlock) {
