...
public class MLGridBearingLocaliser implements BearingLocaliser {
	...
	public MLGridBearingLocaliser(int hydrophoneBitMap, double timingError) {
		prepare(PamUtils.getChannelArray(hydrophoneBitMap), timingError);
	}
	
	public MLGridBearingLocaliser(int[] hydrophoneList, double timingError) {
		prepare(hydrophoneList, timingError);
	}
	
	@Override
	public void prepare(int[] arrayElements, double timingError) {
		prepare(arrayElements, timingError, Math.toRadians(1), Math.toRadians(1));
	}
	
	synchronized private void prepare(int[] arrayElements, double timingError, double thetaStep, double phiStep) {
		...
		for (int i = 0; i < arrayAxis.length; i++) {
			pairBearingLocaliserPairs[i] = findPairLocaliser(arrayAxis[i]);
			if (pairBearingLocaliserPairs[i] >= 0) {
				...
				pairBearingLocalisers[i] = new PairBearingLocaliser(map, timingError);
			}
		}
		...
		for (int iP = 0; iP < nPairs; iP++) {
			pV0 = currentArray.getAbsHydrophoneVector(phonePairs[iP][0]);
			pV1 = currentArray.getAbsHydrophoneVector(phonePairs[iP][1]);
			if (pV1 == null || pV0 == null) {
				continue;
			}
			pairVector = pV1.sub(pV0);
			pairErrorVector = currentArray.getSeparationErrorVector(phonePairs[iP][1], phonePairs[iP][0]);
...
//		System.out.println(String.format("LUT Creation time %3.2f microseconds", (nanoEnd-nanosStart)/1000.));
	...
	private int findPairLocaliser(PamVector pamVector) {
		...
		for (int i = 0; i < nPairs; i++) {
			pairVector = currentArray.getAbsHydrophoneVector(phonePairs[i][0])
			.sub(currentArray.getAbsHydrophoneVector(phonePairs[i][1]));
	...
	@Override
	synchronized public double[][] localise(double[] delays) {
...
//		long t3 = System.nanoTime();
		double[][] angles =  localiseByCrawl(delays);
...
//		long endNanos;
		...
		for (int i = 0; i < 2; i++) {
			...
			angData = pairBearingLocalisers[i].localise(aDelay);
			if (angData != null) {
				if (i == 0) {
