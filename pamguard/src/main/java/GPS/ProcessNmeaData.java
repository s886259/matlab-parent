...
//		}
	...
	public void newData(PamObservable o, PamDataUnit pamData) {
		...
		if (stringId.equalsIgnoreCase(wantedString)) {
			gpsData = new GpsData(nmeaString, gpsController.gpsParameters.mainString, true); // GpsData constructor which
			// unpacks the string.
			if (gpsData.isDataOk()) {
...
//				System.out.println(String.format("Magnetic variation = %3.1f", magVar));
				...
				System.out.println("Bad GPS String : " + nmeaString);
			}
		}
	}
	...
	public void setWantedString() {
		if (gpsController.gpsParameters.mainString == GPSParameters.READ_GGA) {
			wantedString = String.format("$%sGGA", gpsController.gpsParameters.ggaInitials);
		}
		else {
			wantedString = String.format("$%sRMC", gpsController.gpsParameters.rmcInitials);
		}
	}
}
