...
public class FilterDialog extends PamDialog implements ActionListener {

	private FilterParams filterParams = new FilterParams();

	private float sampleRate;

	private String[] filterNames = { "None", "Butterworth", "Chebyshev", "FIR Filter"};
	...
	public boolean getParams() {
		...
		if (filterParams.filterBand == FilterBand.BANDPASS || 
			...
			if (filterParams.lowPassFreq > niquist) {
				return showWarning("The low pass cut off frequency is too high");
			}
		}
		if (filterParams.filterBand == FilterBand.BANDPASS  || 
				filterParams.filterBand == FilterBand.BANDSTOP || 
			...
			if (filterParams.highPassFreq > niquist) {
				return showWarning("The high pass cut off frequency is too high");
			}
		}
...
//			double yScale = (r.getHeight()-margin*2) / 2;
	...
	class BodePlot extends JPanel {
		...
		BodePlot() {
			...
			yAxis = new PamAxis(10, 0, 10, 100, 0, -70, true, "Gain (dB)",
					"%.0f");
...
//				freqPoints[i] = Math.pow(10., logf);
		...
		int yGainCoord(double gain) {
			...
			return (int) ((1 - (db - yScaleMin) / (yScaleMax - yScaleMin)) * boundsRect.height);
		}
	}
	...
	public static FilterParams showDialog(Window ownerFrame, FilterParams filterParams, float sampleRate) {
		...
		singleInstance.filterParams = filterParams.clone();
		singleInstance.sampleRate = Math.max(sampleRate, 1.f);
		singleInstance.setParams();
		singleInstance.setVisible(true);
		return singleInstance.filterParams;
	}
}
