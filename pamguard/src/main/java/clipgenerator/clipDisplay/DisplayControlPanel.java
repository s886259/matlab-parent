...
public class DisplayControlPanel {
	...
	public DisplayControlPanel(ClipControl clipControl,
...
//		scalePanel.add(showTrigger = new PamCheckBox("Show trigger data"));
		...
		if (PamController.getInstance().getRunMode() != PamController.RUN_PAMVIEW) {
			...
			ScrollPaneAddon sco = new ScrollPaneAddon(clipDisplayPanel.getScrollPane(), clipControl.getUnitName(),
					AbstractPamScroller.HORIZONTAL, 1000, 3600*1000, true);
			sco.addDataBlock(clipControl.getClipProcess().getClipDataBlock());
			c.gridwidth = 1;
			c.gridx = 1;
...
//			historyPanel.add(viewerEnd = new PamLabel("1970-01-01 12:00:00"));
	...
	void setValues(ClipDisplayParameters clipParams) {
		...
		logFFT.setValue(clipParams.logFFTLength);
	...
	boolean getValues(ClipDisplayParameters clipParams) {
		...
		clipParams.logFFTLength = (int) (logFFT.getValue() + 0.01);
