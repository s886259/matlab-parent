 ...
 *
 */
public class PamTableDefinition extends EmptyTableDefinition {
	...
	public PamTableDefinition(String tableName, int updatePolicy) {
		super(tableName);
		pamTableItems.add(timeStampItem = new PamTableItem("UTC", Types.TIMESTAMP));
	...
	public String getSQLSelectString(SQLTypes sqlTypes, PamViewParameters pamViewParameters) {
		...
		if (pamViewParameters != null) {
			String b = getBetweenString(pamViewParameters);
			if (b != null) {
				sqlString += b;
			}
		}
	 ...
	 */
	public String getBetweenString(PamViewParameters pvp) {
		...
		if (pvp.viewStartTime != 0 && pvp.viewEndTime != 0) {
			str = String.format(" WHERE UTC BETWEEN %s AND %s", PamCalendar.formatDBDateTimeQueryString(pvp.getRoundedViewStartTime()),
					PamCalendar.formatDBDateTimeQueryString(pvp.getRoundedViewEndTime()));
		}
		else if (pvp.viewStartTime != 0) {
			str = String.format(" WHERE UTC > %s ", PamCalendar.formatDBDateTimeQueryString(pvp.getRoundedViewStartTime()));
		}
		else if (pvp.viewEndTime != 0) {
			str = String.format(" WHERE UTC < %s ", PamCalendar.formatDBDateTimeQueryString(pvp.getRoundedViewEndTime()));
		}
		if (pvp.useAnalysisTime) {
			...
			if (pvp.analStartTime != 0 && pvp.analEndTime != 0) {
				str += String.format(" (LocalTime BETWEEN %s AND %s OR LocalTime BETWEEN %s AND %s)", 
						PamCalendar.formatDBDateTimeQueryString(pvp.getRoundedAnalStartTime()),
						PamCalendar.formatDBDateTimeQueryString(pvp.getRoundedAnalEndTime()),
						PamCalendar.formatDBDateTimeQueryString(pvp.getRoundedViewStartTime()),
						PamCalendar.formatDBDateTimeQueryString(pvp.getRoundedViewEndTime()));
			}
	...
	public Connection getCheckedConnection() {
		return checkedConnection;
	}

	public void setCheckedConnection(Connection checkedConnection) {
