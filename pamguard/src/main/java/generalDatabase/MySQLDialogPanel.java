...
public class MySQLDialogPanel implements SystemDialogPanel {
	...
	public MySQLDialogPanel(Component parent, MySQLSystem mySQLSystem) {
		super();
		this.parent = parent;
		this.mySQLSystem = mySQLSystem;
	}
	...
	public JPanel createPanel() {
		...
		JPanel t = new JPanel();
		t.setBorder(new TitledBorder("MySQL server settings"));
//		t.add(new JLabel("For MySQL databases use the old database module"));
		...
		JPanel b = new JPanel();
		b.setBorder(new TitledBorder("MySQL Database"));
//		b.add(new JLabel("Only use the ODBC databases module for MS Access databases"));
	...
	public boolean getParams() {
		tempParams = mySQLSystem.mySQLParameters.clone();
		...
		mySQLSystem.mySQLParameters = tempParams.clone();
		return true;
	}
	...
	public void setParams() {
		ipAddress.setText(mySQLSystem.mySQLParameters.ipAddress);
		userName.setText(mySQLSystem.mySQLParameters.userName);
		passWord.setText(mySQLSystem.mySQLParameters.passWord);
		portNumber.setText(String.format("%d",mySQLSystem.mySQLParameters.portNumber));
	...
	public void setDbParams(boolean newList) {
		...
		ArrayList<String> dbList = mySQLSystem.getAvailableDatabases(newList);
		int selIndex = -1;
		if (dbList != null) for (int i = 0; i < dbList.size(); i++) {
			databaseList.addItem(dbList.get(i));
			if (dbList.get(i).equalsIgnoreCase(mySQLSystem.mySQLParameters.databaseName)) {
				selIndex = i;
			}
		}
	...
	class ConnectServer implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			mySQLSystem.serverConnect(tempParams);
			sayServerStatus();
		}
	...
	void sayServerStatus() {
		if (mySQLSystem.isServerConnected()) {
			connectionStatus.setText("Connected");
		}
		else  {
			connectionStatus.setText("No Connection");
		}
		if (lastStatus != mySQLSystem.isServerConnected()) {
			setDbParams(true);
			lastStatus = mySQLSystem.isServerConnected();
		}
		enableControls();
	}
	
	void enableControls() {
		boolean ok = mySQLSystem.isServerConnected();
...
//			}
			...
			if (mySQLSystem.createNewDatabase(defaultName)) {
				setDbParams(true);
			}
		}
