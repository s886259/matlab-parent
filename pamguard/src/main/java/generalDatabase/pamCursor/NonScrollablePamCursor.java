...
//	}
	...
	@Override
	public boolean openScrollableCursor(Connection connection, boolean includeKeys,
			boolean includeCounters, String clause) {
		setCurrentConnection(connection);
		String sqlString = this.getSelectString(getSQLTypes(connection), includeKeys, includeCounters, clause);
...
//				iRow++;
	...
	@Override
	int getLastDatabaseIndex(Connection connection,
...
//				System.out.println("Database insert result = " + ans);
	...
	private boolean deleteUnwanted() {
		...
		delString += ")";
		try {
			PreparedStatement pStmt = connection.prepareStatement(delString);
			pStmt.execute();
			pStmt.close();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public int findColumn(String columnLabel) {
		for (int i = 0; i < getTableDefinition().getTableItemCount(); i++) {
			if (getTableDefinition().getTableItem(i).getName().equalsIgnoreCase(columnLabel)) {
				return i; // may need to change this to i+1 for compatibility !
			}
		}
		return 0;
	}
