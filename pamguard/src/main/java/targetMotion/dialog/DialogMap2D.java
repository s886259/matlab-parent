...
public class DialogMap2D<T extends PamDetection> extends DialogMap<T> {
	...
	private PamSymbol bestHighlight = new PamSymbol(PamSymbol.SYMBOL_CIRCLE, 20, 20, false, Color.WHITE, Color.LIGHT_GRAY);
...
//		}
	...
	private void sortPlotDimensions() {
		...
		PamDetection pd = currentEvent.getSubDetection(0);
		if (pd == null) {
			return;
		}
...
//			System.out.println(String.format("Key %d mods %d are %s", e.getKeyCode(), mods, txt));
	...
	class MapPlotPanel extends PamPanel {
		...
		public MapPlotPanel() {
			...
			originSymbol = new PamSymbol(PamSymbol.SYMBOL_CIRCLE, 3, 3, true, Color.BLUE, Color.blue);
			resultSymbol = new PamSymbol(PamSymbol.SYMBOL_HEXAGRAM, 9, 9, true, Color.BLUE, Color.RED);
