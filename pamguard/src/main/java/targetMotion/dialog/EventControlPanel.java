...
public class EventControlPanel<T extends PamDetection> implements TMDialogComponent{
...
//		c.gridy = y;
		...
		c.gridwidth = 1;
		c.gridy++;
		PamDialog.addComponent(mainPanel, runButton = new JButton("Run"), c);
		c.gridx++;
		PamDialog.addComponent(mainPanel, runAllButton = new JButton("Run All"), c);
		c.gridx++;
...
//		T currentEvent = getListedEvent();
	...
	public void selectEventIndex(int selEventDBIndex) {
		...
		TargetMotionLocalisation tml = (TargetMotionLocalisation) selEvent.getLocalisation();
		if (tml != null) {
...
//	}
	...
	private void sayEventInfo(T event) {
		...
		TargetMotionLocalisation loc  = null;
		try {
			loc = (TargetMotionLocalisation) event.getLocalisation();
		}
