...
//	private NoiseControl noiseControl;
	...
	private int[] symbolTypes = {PamSymbol.SYMBOL_CIRCLE, PamSymbol.SYMBOL_SQUARE, 
			PamSymbol.SYMBOL_TRIANGLEU, PamSymbol.SYMBOL_TRIANGLED, PamSymbol.SYMBOL_DIAMOND,
			PamSymbol.SYMBOL_STAR, PamSymbol.SYMBOL_TRIANGLEU};
...
//		NoiseSettings ns = getCurrentNoiseSettings();
		...
		timeScroller.setVisibleAmount(noiseDisplaySettings.displayLengthSeconds*1000);
		selectionChanged();
		autoFreqScale();
	}
...
//						continue;
					...
					for (int m = 0; m < nBands; m++) {
						max = Math.max(max, noiseData[m][i]);
						min = Math.min(min, noiseData[m][i]);
						haveData++;
					}
				}
			}
	...
	private class NoiseObserver implements PamObserver {
	...
	private class SpecPlotAxesPanel extends PamAxisPanel {
		...
		public SpecPlotAxesPanel() {
			...
			specLevelAxis = new PamAxis(0, 1, 0, 1, noiseDisplaySettings.levelMin, 
					...
					"Level (dB)", PamAxis.LABEL_NEAR_CENTRE, "%3.1f");
	...
	private class SpecPlotPanel extends JPanelWithPamKey {
		...
		PamSymbol symbol = new PamSymbol(PamSymbol.SYMBOL_CIRCLE, 10, 10, true, Color.RED, Color.RED);
...
//			sortScales();
			...
			for (int iChan = 0; iChan <= hChan; iChan++) {
				...
				for (int i = 0; i < nBands; i++) {
					...
					for (int iM = 0; iM < nStats; iM++) {
						y[iM] = (int) specLevelAxis.getPosition(noiseData[i][iM]);
					}
	...
	private class PlotAxesPanel extends PamAxisPanel {
		public PlotAxesPanel() {
			...
			timeScroller = new PamScroller(pamControlledUnit.getUnitName(), 
					AbstractPamScroller.HORIZONTAL, 1000, 15*60*1000, isViewer);
	...
	private class PlotPanel extends JPanelWithPamKey {
		...
		public void createKey() {
			...
			PamSymbol aSymbol = new PamSymbol(PamSymbol.SYMBOL_DIAMOND, 10, 10, true, 
			...
			int chanNum;
			int nThings = dataSelectionPanel.checkBoxes.length;
			aSymbol.setSymbol(PamSymbol.SYMBOL_LINESEGMENT);
			String txt;
