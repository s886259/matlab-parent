...
public class NoiseControl extends PamControlledUnit implements PamSettings {
	...
	protected boolean fillXMLParameters(Document doc, Element paramsEl) {		
		addXMLParameter(doc, paramsEl, noiseSettings.channelBitmap, "channelBitmap", 
				"Bitmap of used channels", 0, 0);		
		addXMLParameter(doc, paramsEl, noiseSettings.measurementIntervalSeconds, "measurementIntervalSeconds", 
				"Interval between measurements (s)", 0, 0);		
		addXMLParameter(doc, paramsEl, noiseSettings.nMeasures, "nMeasures", 
				"Number of FFT slices to use in measurement", 0, 0);		
		addXMLParameter(doc, paramsEl, noiseSettings.useAll, "useAll", 
				"Use all FFT data", 0, 0);
		int nMeasures = noiseSettings.getNumMeasurementBands();
		
		Element bandsElement = doc.createElement("Bands");
		Element el;
		NoiseMeasurementBand aBand;
		for (int i = 0; i < nMeasures; i++) {
			 aBand = noiseSettings.getMeasurementBand(i);
			 el = doc.createElement("Band");
			 addXMLParameter(doc, el, aBand.name, "name", "Name", 0, 0);
			 addXMLParameter(doc, el, aBand.f1, "f1", "Low frequency", 0, 0);
			 addXMLParameter(doc, el, aBand.f2, "f2", "High frequency", 0, 0);
			 addXMLParameter(doc, el, aBand.getType(), "type", "(0 = userdefined; 1 = third octave)", 0, 0);
			 bandsElement.appendChild(el);
		}
		paramsEl.appendChild(bandsElement);
		return true;
	}
}
