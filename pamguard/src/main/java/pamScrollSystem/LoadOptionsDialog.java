...
public class LoadOptionsDialog extends PamDialog {
	...
	private LoadOptionsDialog(Window parentFrame, AbstractPamScroller scroller, JComponent parentComponent) {
		...
		c.gridx++;
		c.gridwidth = 2;
		addComponent(panel, startTime = new JTextField(16), c);
		c.gridx = 0;
		c.gridwidth = 1;
		c.gridy++;
		...
		c.gridx++;
		addComponent(panel, durationButton = new JButton(" "), c);
		c.gridx = 0;
		c.gridy++;
...
//			if (loadTime =
	...
	private void setParams(AbstractPamScroller scroller) {
		...
		minDuration = scrollerData.stepSizeMillis*1000;
		maxDuration = minDuration * 100000;
	}
