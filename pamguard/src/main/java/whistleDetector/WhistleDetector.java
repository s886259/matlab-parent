...
//				peakDetector);
		...
		whistleDataBlock.setLocalisationContents(AbstractLocalisation.HAS_AMBIGUITY | AbstractLocalisation.HAS_BEARING);
...
//				peakDetectors[i] = new BetterPeakDetector(whistleControl, this, (PamFFTProcess) getParentProcess(), groupChannels);
		...
		for (int i = 0; i < nChannelGroups; i++) {
			...
			if (PamUtils.getNumChannels(groupChannels) > 1) {
				whistleDataBlock.addLocalisationContents(AbstractLocalisation.HAS_BEARING);
			}
		}
	}
...
//		send it to the appropriate detector. 
		...
		if (peakDetectors != null) for (int i = 0; i < peakDetectors.length; i++) {
			if ((peakDetectors[i].getGroupChannels() & obj.getChannelBitmap()) != 0) {
				peakDetectors[i].update(obs, obj); // save the peak detector having to suscribe itself.
	...
	private void prepareEqualisedSpectrumData(FFTDataUnit fftData, double[] background) {
		...
		if (eDataUnit == null) {
			eDataUnit = new FFTDataUnit(fftData.getTimeMilliseconds(), fftData.getChannelBitmap(), 
					fftData.getStartSample(), fftData.getDuration(), null, fftData.getFftSlice());
		}
		else {
			eDataUnit.setInfo(fftData.getTimeMilliseconds(), fftData.getChannelBitmap(), 
					fftData.getStartSample(), fftData.getDuration(), fftData.getFftSlice());
		}
		Complex[] sourceData = fftData.getFftData();
		Complex[] data = eDataUnit.getFftData();
		if (data == null || data.length != sourceData.length) {
			data = Complex.allocateComplexArray(sourceData.length);
		}
		for (int i = 0; i < data.length; i++) {
			data[i].real = Math.sqrt(Math.max(0, sourceData[i].magsq() - background[i]));
		}
...
//		}
	...
	boolean newWhistleEmbryo(WhistleShape newWhistle, int groupChannels, int detectionChannel) {
		...
		newWhistle.setDBAmplitude(daqProcess.fftAmplitude2dB(newWhistle.getAmplitude(), 
				detectionChannel, fftLength, true, false));
...
//				- startSample, 1 << iChan);
		...
		// calculate bearings / delays if there is > 1 channel in the group
		if (PamUtils.getNumChannels(groupChannels) > 1) {
			localiseWhistle(sdu);
		}
