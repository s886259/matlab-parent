...
public class WhistleLocaliser {
	...
	public int getDelay(FFTDataBlock fftDataBlock, WhistleShape whistle, int whistleChannel, int bearingChannel) {
		...
		if (complexData == null || complexData.length != fftLength) {
			complexData = new Complex[fftLength];
		...
		fastFFt.ifft(complexData, FastFFT.log2(fftDataBlock.getFftLength()));
		...
		double sumSq = 0;
		for (int i = 0; i < fftLength; i++) {
			if (complexData[i].real > corrMax) {
				corrMax = complexData[i].real;
				delay = i;
			}
			sumSq += complexData[i].magsq();
		}
