...
//				IshDetFnDataUnit.class, getLongName(), this, 1 << channel);
		...
		outputDataBlock.setLocalisationContents( 
				AbstractLocalisation.HAS_BEARING | AbstractLocalisation.HAS_AMBIGUITY | 
				AbstractLocalisation.HAS_XY | AbstractLocalisation.HAS_XYZ);
		//Set up data logging.
...
//		outputDataBlock.setOverlayDraw(new PamDetectionOverlayGraphics(outputDataBlock));
	...
	public PamDataBlock getInputDataBlock() {
		...
		return (p == null || p.inputDataSource == null) ? getParentDataBlock() :
			PamController.getInstance().getDataBlock(inputDataClass(), p.inputDataSource);
	}
...
//	public IshDetFnDataUnit getOutputDataUnit(FFTDataUnit fftDataUnit) {}
	public IshDetFnDataUnit getOutputDataUnit(AcousticDataUnit rawOrFftDataUnit) {
		...
		long dur            = rawOrFftDataUnit.getDuration();
