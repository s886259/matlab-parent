...
public class PamSymbolDialog extends PamDialog implements ActionListener {
	...
	public PamSymbolDialog(Window parentFrame) {
		...
		JPanel g = new JPanel();
		g.setBorder(new TitledBorder("Fill Colour"));
		//		e.add(new JLabel("Fill Color"));
		g.add(fillColorChooser = new JColorChooser());
		e.add(f);
		e.add(g);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == symbolButton) {
			...
			int symbolType;
			try {
				symbolType = Integer.valueOf(((JMenuItem) e.getSource())
						.getActionCommand());
	...
	private void enableControls() {
		...
		fillColorChooser.setEnabled(givenSymbol.isSolidShape());
	...
	class SamplePanel extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			givenSymbol.draw(g, new Point(w / 2 - size / 2
					+ givenSymbol.getWidth(), h / 2 - size / 2
					- givenSymbol.getHeight() / 2),
					givenSymbol.getWidth(), givenSymbol.getHeight());
...
//		if (typeMenu == null) {
			...
			addTypeItem(typeMenu, PamSymbol.SYMBOL_CIRCLE);
			addTypeItem(typeMenu, PamSymbol.SYMBOL_SQUARE);
			addTypeItem(typeMenu, PamSymbol.SYMBOL_POINT);
			addTypeItem(typeMenu, PamSymbol.SYMBOL_TRIANGLEU);
			addTypeItem(typeMenu, PamSymbol.SYMBOL_TRIANGLED);
			addTypeItem(typeMenu, PamSymbol.SYMBOL_TRIANGLEL);
			addTypeItem(typeMenu, PamSymbol.SYMBOL_TRIANGLER);
			addTypeItem(typeMenu, PamSymbol.SYMBOL_DIAMOND);
			addTypeItem(typeMenu, PamSymbol.SYMBOL_STAR);
			addTypeItem(typeMenu, PamSymbol.SYMBOL_CROSS);
			addTypeItem(typeMenu, PamSymbol.SYMBOL_CROSS2);
			addTypeItem(typeMenu, PamSymbol.SYMBOL_PENTAGRAM);
			addTypeItem(typeMenu, PamSymbol.SYMBOL_HEXAGRAM);
//		}
		// MouseInfo.getPointerInfo().
		typeMenu.show(e, 0, 0);
	}

	private void addTypeItem(JPopupMenu menu, int symbolType) {
		JMenuItem item;
...
//		pamSymbol.setSymbol(symbolType);
		...
		item.setActionCommand(String.format("%d", symbolType));
		menu.add(item);
	}
}
