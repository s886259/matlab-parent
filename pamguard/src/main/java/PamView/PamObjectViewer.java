...
public class PamObjectViewer implements PamViewInterface, ComponentListener,
	...
	static public PamObjectViewer getObjectViewer() {
		if (singleInstance == null) {
			singleInstance = new PamObjectViewer();
		}
		return singleInstance;
	}

	static public void Show() {
		getObjectViewer().objectFrame.setVisible(true);
	}
...
//			setIconImages(getOwner().getIconImages());
	...
	class LayoutPanel extends PamDesktopPane {
			...
			for (int i = 0; i < controllerList.size(); i++) {
				...
				for (int iP = 0; iP < pamControlledUnit.getNumPamProcesses(); iP++) {
					...
					if (sourceControllerView == pamControllerView) {
						orig = sourceControllerView
								.getBlockLeftPos(sourceBlock);
					} else {
						orig = sourceControllerView
								.getBlockRightPos(sourceBlock);
					}
					...
					if (sourceControllerView == pamControllerView) {
						drawIntArrow(g, orig.x, orig.y, dest.y, 6 + iP * 2, 6);
					} else {
						drawArrow(g, orig.x, orig.y, dest.x, dest.y, bestYGap,
								6);
					}
				}
			}
		...
		private void drawArrow(Graphics g, int x1, int y1, int x2, int y2,
				int bestYGap, int headSize) {
			drawSteppedArrow(g, x1, y1, x2, y2, bestYGap, headSize);
		}

		private void drawSteppedArrow(Graphics g, int x1, int y1, int x2,
				int y2, int bestYGap, int headSize) {
			int xMin = 16;
			int xm = (x1 + x2) / 2;
			if (x2 - x1 >= xMin) {
				...
				drawStraightArrow(g, xm, y2, x2, y2, headSize);
			} else {
				...
				drawStraightArrow(g, x2 - xMin / 2, y2, x2, y2, headSize);
			}
		}
	...
	class PamControllerView extends JInternalFrame {
		...
		public Point getProcessLeftPos(int processNum) {
			Point pt = new Point();
			pt.x = this.getX();
		...
		public Point getBlockLeftPos(int processNum, int blockNum) {
			Point pt = new Point();
			pt.x = this.getX();
		...
		public Point getBlockRightPos(int processNum, int blockNum) {
			Point pt = new Point();
		...
		public Point getBlockLeftPos(PamDataBlock dataBlock) {
			Point pt = new Point();
			pt.x = this.getX();
		...
		public Point getBlockRightPos(PamDataBlock dataBlock) {
			Point pt = new Point();
			pt.x = this.getX() + this.getWidth();
			Component c = findComponent(dataBlock);
			pt.y = this.getY() + getYOffset();
...
//					processButton.set
			...
			protected void updateCpuUsage() {
				if (processText != null) {
					processText.setText(String.format(("%3.1f %%"), pamProcess
							.getCpuPercent()));
				}
			}
...
//				}
	...
	class PamProcessView extends JInternalFrame {
		...
		void FillProcessContent() {
			...
			constraints.gridx = 1;
			addComponent(p, cpuTime = new JTextField(3), constraints);
			cpuTime
					.setToolTipText("Process CPU Usage (includes observers CPU)");
			...
			for (int i = 0; i < pamProcess.getNumOutputDataBlocks(); i++) {
				...
				// constraints.fill = GridBagConstraints.HORIZONTAL;
				addComponent(p,
						label = new JButton(pamDataBlock.getDataName()),
						constraints);
				label.setToolTipText("Pam DataBlock : "
				...
				dataBlockInfoList.add(viewGroupInfo = new ViewGroupInfo(this,
						label, pamDataBlock, unitCount));
				label.addActionListener(new DataBlockListener(pamDataBlock));
		...
		Timer blockTimer = new Timer(1000, new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				// fill in the CPU usage
				cpuTime.setText(String.format(("%3.1f %%"), pamProcess
						.getCpuPercent()));
	...
	JPopupMenu getObjectViewerMenu() {
		...
		layoutMenuItem.addActionListener(getObjectViewer().objectFrame);
		...
		viewerMenu.add(PamModuleInfo
				.getModulesMenu(getObjectViewer().objectFrame));
		return viewerMenu;
	}
