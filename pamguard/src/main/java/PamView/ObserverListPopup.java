...
//		setAlwaysOnTop(true);
	...
	public void setDataBlock(PamDataBlock dataBlock) {
		...
		constraints.gridx = 2;
		addComponent(observerPanel, new JLabel("CPU"), constraints);
		constraints.gridy ++;
		for (int i = 0; i < dataBlock.countObservers(); i++) {
			constraints.gridx = 0;
			constraints.anchor = GridBagConstraints.WEST;
			addComponent(observerPanel, new JLabel(observers.get(i).getObserverName()), constraints);
			constraints.gridx = 1;
			constraints.anchor = GridBagConstraints.CENTER;
			str = String.format("%.1f s", (double) observers.get(i).getRequiredDataHistory(dataBlock, null)/1000);
