 ...
 * firswt of all digitally filtering, then downsampling
...
//	private long sampleCounts[];
	...
	DecimatorProcess(DecimatorControl controlUnit){//), PamRawDataBlock sourceData) {
...
//		sampleCounts = new long[PamConstants.MAX_CHANNELS];
	...
	public void newData(PamObservable obs, PamDataUnit newData) {
		...
		if (outputBlockSize == 0) {
			 ...
			 * is never less than the input block, otherwise we will lost the interleaving 
		...
		while (true) {
			...
			if (outputIndex[chan] == outputBlockSize) {
				...
				//				long millis = absSamplesToMilliseconds(outputSampleNumber[chan]);
				long millis = rawDataUnit.getTimeMilliseconds();
				millis += (b1[chan]-outputBlockSize) * 1000 / getSampleRate();
				if (newDataUnit != null) {
					newDataUnit.setInfo(millis, rawDataUnit.getChannelBitmap(), 
							outputSampleNumber[chan], outputBlockSize);
				}
				else {
					newDataUnit = new RawDataUnit(millis, rawDataUnit.getChannelBitmap(), 
							outputSampleNumber[chan], outputBlockSize);
				}
...
//	}
	...
	public int getOfflineData(PamDataBlock dataBlock, PamObserver endUser, long startMillis,
		...
		if (decimatorControl.getOfflineFileServer().loadData(getOutputDataBlock(), startMillis, 
