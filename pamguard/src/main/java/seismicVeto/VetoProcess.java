...
public class VetoProcess extends PamProcess {
	...
	public VetoProcess(VetoController vetoController) {
		...
		detectionOverlayGraphics.setPamSymbol(new PamSymbol(PamSymbol.SYMBOL_SQUARE, 10, 10, true, Color.GREEN, Color.RED));
...
//			System.out.println("No raw data from unit at sample " + ((RawDataUnit) arg).getStartSample());
		...
		if (newUnit == null) {
			newUnit = new RawDataUnit(rawDataUnit.getTimeMilliseconds(), rawDataUnit.getChannelBitmap(), 
					rawDataUnit.getStartSample(), rawDataUnit.getDuration());
		}
		else {
		...
		if (newData == null || newData.length != rawDataUnit.getDuration()) {
			newData = new double[(int) rawDataUnit.getDuration()];
		}
...
//		}
		...
		Complex[] oldData = fftDataUnit.getFftData();

		Complex[] newData = newFFTUnit.getFftData();
		if (newData == null) {
			newData = fftOutputData.getComplexArray(oldData.length);
		}
		...
		if (isVetoed(chan, fftDataUnit.getStartSample())) {
			...
			if (vetoController.vetoParameters.randomFillSpectorgram) {
				...
				for (int i = 0; i < oldData.length; i++) {
					if (newData[i] == null) {
						newData[i] = new Complex(Math.sqrt(referenceData[i]),0);
					}
					else {
						newData[i].assign(Math.sqrt(referenceData[i]),0);
					}
				}
			}
			else {
				for (int i = 0; i < oldData.length; i++) {
					if (newData[i] == null) {
						newData[i] = new Complex();
					}
					else {
						newData[i].assign(0,0);
					}
				}
			}
		}
...
//				b1 = 0;
			...
			if (isVetoed(channel, arg.getStartSample()) == false) {
				Complex[] fftData = arg.getFftData();

				for (int i = 0; i < runningBackground.length; i++) {
					runningBackground[i] *= b2;
					runningBackground[i] += fftData[i].magsq() * b1;
				}
			}
...
//			newDataunit.frequency[1] = workshopController.workshopProcessParameters.highFreq;
			...
			wdu.setMeasuredAmplitude(daqProcess.fftAmplitude2dB(aveAmplitude, 
					channel, fftInputData.getFftLength(), true, false), AcousticDataUnit.AMPLITUDE_SCALE_DBREMPA);
