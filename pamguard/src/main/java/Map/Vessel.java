...
public class Vessel extends PamSymbol implements Serializable, Cloneable {
...
//		shipColor = vesselColor;
		...
		shipGps = new GpsData();
		setSymbol(PamSymbol.SYMBOL_CUSTOMPOLYGON);
	}
...
//		shipOutline[4].y = shipOutline[0].y;
		...
		for (int i = 0; i < shipOutline.length; i++) {
			...
			tempCoord = TransformUtilities.rotationDegreesXYZ(tempCoord,
					shipGps.getHeading());
