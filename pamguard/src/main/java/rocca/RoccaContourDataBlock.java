...
//
	...
	public RoccaContourDataBlock(PamProcess parentProcess, int channelMap) {
        ...
        this.treeVotes = new double[roccaProcess.roccaControl.roccaSidePanel.getSpeciesClassCount().length];
...
//
        ...
        double startTime = this.getDataUnit(firstUnitWithWhistle, REFERENCE_CURRENT).getTime();
...
//        		}
        ...
        contourStats.put(RoccaContourStats.ParamIndx.DURATION, (rcdu.getTime()-startTime)/1000);
...
//            Arrays.sort(smallArray);
        ...
        if (contourStats.get(RoccaContourStats.ParamIndx.NUMINFLECTIONS) > 1) {
	        ...
	        for (int j=numPoints-2; j>numPoints-numInfl-1; j--) {
	        	smallArray[count]=(inflTimeArray[numPoints-1]-inflTimeArray[j])/1000;
	        	count++;
	        }
	        Arrays.sort(smallArray);
      ...
      // the code below would work with either method of inflection time calculation
            ...
            contourStats.put(RoccaContourStats.ParamIndx.INFLMAXDELTA, 999.0);
            contourStats.put(RoccaContourStats.ParamIndx.INFLMINDELTA, 999.0);
            contourStats.put(RoccaContourStats.ParamIndx.INFLMAXMINDELTA, 999.0);
            contourStats.put(RoccaContourStats.ParamIndx.INFLMEANDELTA, 999.0);
            contourStats.put(RoccaContourStats.ParamIndx.INFLSTDDEVDELTA, 999.0);
            contourStats.put(RoccaContourStats.ParamIndx.INFLMEDIANDELTA, 999.0);
        }
    ...
    public String createContourStatsHeader() {
        String contourHeader = 
                "Source, Date-Time, DetectionCount, DetectionNumber, ClassifiedSpecies, ";
        ...
        contourHeader = contourHeader + "Classifier";
...
//        contourHeader = contourHeader.substring(0, contourHeader.length()-2);
     ...
     * school classification.  This method sets the filename equal to the raw
     ...
     * and school classification.  This method sets the filename equal to the
    ...
    public String createContourStatsString(String clipFile, int detectionCount) {
        ...
        contourStatsStr =
                ...
                detectionCount +
                ", " +
                roccaProcess.roccaControl.roccaSidePanel.getSightingNum() +
                ", " +
                classifiedAs +
                ", " +
                contourStatsStr;
        ...
        /* add the classifier */
        contourStatsStr = contourStatsStr +
                roccaProcess.roccaControl.roccaParameters.getRoccaClassifierModelFilename().getName() +
                ", ";

        /* add the tree votes.  Also save the tree votes as a single String with
         * the '-' character separating the values, to save to the database
         */
        String voteList = "(";
...
//        contourStats = contourStats.substring(0, contourStats.length()-2);
        ...
        RoccaLoggingDataUnit rldu = new RoccaLoggingDataUnit(
                this.getFirstUnit().getTimeMilliseconds(), rcs);
