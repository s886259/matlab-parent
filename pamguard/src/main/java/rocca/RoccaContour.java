...
//                System.out.println(String.format("   Raw Data size = %d", currentRawData[0].length));
                ...
                if ((nextFFTStartTime < contourStartTime) ||
                    ...
                    currentFFTDataUnit.getDuration() );
                 ...
                 */
                     ...
                     * the contour so find the peak frequency within the senseLow
                     * and senseHigh bin
                     */
                    } else {
         ...
         //               System.out.println(String.format("   Higher Index = %d", upperFreqBinIndex));
                    ...
                    /*
                    currentMeanTime = ( currentFFTDataUnit.getStartSample() +
                                    ( currentRawData[0].length/2 )      ) / rawDataSamplingRate;
                     */

                    //System.out.println(String.format("   Time = %f", currentMeanTime));
    ...
    private int getMaxFreqIndexFromFFTData(Complex[] currentFFTData, int lowerFreqBinIndex, int upperFreqBinIndex) {
        ...
        double maxFFT = currentFFTData[lowerFreqBinIndex].real;
        if (upperFreqBinIndex > currentFFTData.length) {
            upperFreqBinIndex = currentFFTData.length;
        }
        for (int i = lowerFreqBinIndex+1; i <= upperFreqBinIndex; i++) {
            if (currentFFTData[i].real > maxFFT) {
                maxFFT = currentFFTData[i].real;
                maxFreqIndex = i;
            }
        }
        return maxFreqIndex;
