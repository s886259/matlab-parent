...
//		}
	...
	public JMenuItem createDetectionMenu(Frame parentFrame) {
		...
		menuItem = new JMenuItem("Recording Settings ...");
		menuItem.addActionListener(new MenuRecordSettings(parentFrame));
		menu.add(menuItem);
		menuEnabler.addMenuItem(menuItem);
...
//		paramsEl.setAttribute("enableBuffer", ((Boolean) recorderSettings.enableBuffer).toString());
//		paramsEl.setAttribute("bufferLength", ((Integer) recorderSettings.bufferLength).toString());
//		paramsEl.setAttribute("outputFolder", recorderSettings.outputFolder);
//		paramsEl.setAttribute("fileInitials", recorderSettings.fileInitials);
//		paramsEl.setAttribute("autoInterval", ((Integer) recorderSettings.autoInterval).toString());
//		paramsEl.setAttribute("autoDuration", ((Integer) recorderSettings.autoDuration).toString());
//		paramsEl.setAttribute("maxLengthSeconds", ((Integer) recorderSettings.maxLengthSeconds).toString());
//		paramsEl.setAttribute("maxLengthMegaBytes", ((Long) recorderSettings.maxLengthMegaBytes).toString());
//		paramsEl.setAttribute("initialState", ((Integer) recorderSettings.oldStatus).toString());
		
		addXMLParameter(doc, paramsEl, "WAV", "Format", 
				"Recording format ('WAV' or 'X3')", 0, 0);
		int availChanns = 0xFFFFFFFF;
		if (recorderProcess.getParentDataBlock() != null) {
			availChanns = recorderProcess.getParentDataBlock().getChannelMap();
		}
		addXMLParameter(doc, paramsEl, recorderSettings.getChannelBitmap(availChanns), "channelBitmap", 
				"Channel Bitmap", 0, 0);
		addXMLParameter(doc, paramsEl, recorderSettings.enableBuffer, "enableBuffer", 
				"Enable buffered recording", 0, 0);
		addXMLParameter(doc, paramsEl, recorderSettings.bufferLength, "bufferLength", 
				"Buffer length (seconds)", 0, 0);
		addXMLParameter(doc, paramsEl, recorderSettings.outputFolder, "outputFolder", 
				"Output Folder", 0, 0);
		addXMLParameter(doc, paramsEl, recorderSettings.fileInitials, "fileInitials", 
				"File Initials (typically 2 or 3 characters)", 0, 0);
		addXMLParameter(doc, paramsEl, recorderSettings.autoInterval, "autoInterval", 
				"Interval between automatic scheduled recordings (seconds)", 0, 0);
		addXMLParameter(doc, paramsEl, recorderSettings.autoDuration, "autoDuration", 
				"Duration of automatic scheduled recordings (seconds)", 0, 0);
		addXMLParameter(doc, paramsEl, recorderSettings.maxLengthSeconds, "maxLengthSeconds", 
				"Max length of a single recording in seconds", 0, 0);
		addXMLParameter(doc, paramsEl, recorderSettings.maxLengthMegaBytes, "maxLengthMegaBytes", 
				"Max length of a single recording in megabytes", 0, 0);
		addXMLParameter(doc, paramsEl, recorderSettings.oldStatus, "initialState", 
				"Initial state (1=off, 2=scheduled, 3=continuous)", 0, 0);
		
		// now add a block of nodes describing autorecord options
		Element arElement = doc.createElement("Triggers");
		Element rtElement;
		RecorderTriggerData rtData;
		for (RecorderTrigger rt:recorderTriggers) {
			rtElement = doc.createElement("Trigger");
			rtData = recorderSettings.findTriggerData(rt);
			rtData.fillXMLParameters(doc, rtElement);
			arElement.appendChild(rtElement);
		}
		paramsEl.appendChild(arElement);
