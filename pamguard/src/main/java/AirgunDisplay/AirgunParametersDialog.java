...
public class AirgunParametersDialog extends PamDialog {
	...
	public AirgunParametersDialog(Frame parentFrame) {
		...
		p.add(shipDrawing = new ShipDimensionsPanel(ad,
				shipDimensionsFields = new ShipDimensionsFields("EF")));
	...
	public boolean getParams() {
		...
		if (dim == null || dim.length != 2) return false;
	...
	class ShipIDPanel extends JPanel implements ActionListener {
		...
		JRadioButton thisVessel, otherVessel;
		JTextField mmsiNumber;
		
		public ShipIDPanel() {
			super();
			setBorder(new TitledBorder("Source Vessel Idenitification"));
			...
			c.gridx = 0;
			c.gridwidth = 1;
			addComponent(this, new JLabel("Source vessel mmsi number"), c);
			c.gridx++;
			addComponent(this, mmsiNumber = new JTextField(12), c);
		}
		...
		public void setParams() {
			thisVessel.setSelected(airgunParameters.gunsThisVessel);
			otherVessel.setSelected(!airgunParameters.gunsThisVessel);
			mmsiNumber.setText(String.format("%d", airgunParameters.gunsMMSIVessel));
			enableControls();
		}
		
		public boolean getParams() {
			airgunParameters.gunsThisVessel = thisVessel.isSelected();
			if (airgunParameters.gunsThisVessel == false) {
				try {
	...
	class ExclusionPanel extends JPanel implements ActionListener {
		...
		public ExclusionPanel() {
			...
			c.gridy ++;
			c.gridwidth = 1;
			addComponent(this, new JLabel("Predict ahead for "), c);
			c.gridx++;
