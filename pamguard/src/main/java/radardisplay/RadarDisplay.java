...
public class RadarDisplay extends UserFramePlots implements PamObserver {
	...
	public RadarDisplay(UserDisplayControl userDisplayControl, RadarParameters radarParameters) {
		...
		if (PamController.getInstance().getRunMode() == PamController.RUN_PAMVIEW) {
			radarScroller = new PamScroller("Radar", AbstractPamScroller.HORIZONTAL, 1000, 600000L, true);
			radarScroller.addObserver(new ScrollObserver());
			radarScroller.setRangeMillis(radarParameters.scrollMinMillis, radarParameters.scrollMaxMillis, false);
			radarScroller.setVisibleAmount((long) (radarParameters.viewRangeSeconds * 1000));
			...
			rangeSpinner.setSpinnerValue(radarParameters.viewRangeSeconds);
	...
	private class ScrollObserver implements PamScrollObserver {
		...
		public void scrollRangeChanged(AbstractPamScroller pamScroller) {
			...
			radarScroller.setVisibleAmount((long) (rangeSpinner.getSpinnerValue() * 1000));
			repaintAll();
		}
	...
	private class ScrollRangeListener implements RangeSpinnerListener {
		...
		public void valueChanged(double oldValue, double newValue) {
			radarScroller.setVisibleAmount((long) (newValue * 1000));
	...
	class RadarPlot extends JPanelWithPamKey {
		...
		private void paintDetectorData(Graphics2D g2d) {
			...
			int n = Math.min(detectorDataBlocks.size(), radarParameters.showDetector.length);
			for (int i = 0; i < n; i++) {
				if (radarParameters.showDetector[i] == false) continue;
				dataBlock = detectorDataBlocks.get(i);
				if (dataBlock.canDraw(radarProjector) == false) continue;
				paintDetectorData(g2d, dataBlock, radarParameters.detectorLifetime[i] * 1000);
			}
		}
		
		private void paintDetectorData(Graphics2D g2d, PamDataBlock dataBlock, long lifetime) {
...
//			int ind = dataBlock.getIndexOfFirstUnitAfter(now - lifetime);
//			if (ind < 0) return;
...
//				rc = String.format("%s - %s", PamCalendar.formatDateTime(start), PamCalendar.formatDateTime(end));
				...
				if (x < (getWidth()/2 + 10)) {
					rc = String.format("%s", PamCalendar.formatTime(end));
					x = getWidth() - g2d.getFontMetrics().stringWidth(rc + " ");
				}
				g2d.drawString(rc, x, y);
			}
...
//			y = g2d.getFontMetrics().getHeight()*3/2;
	...
	class SettingsAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			RadarParameters newParams = RadarParametersDialog.showDialog(
	...
	public long getRequiredDataHistory(PamObservable o, Object arg) {
		...
		int detIndex = detectorDataBlocks.indexOf(dataBlock);
		if (detIndex >= 0 && detIndex < radarParameters.showDetector.length) {
			if (radarParameters.showDetector[detIndex]){
				return radarParameters.detectorLifetime[detIndex]*1000;
			}
		}
		return 0;
	}
