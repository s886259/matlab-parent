...
public class ClickTrigger extends ClickDisplay implements PamObserver {
	...
	public void update(PamObservable obs, PamDataUnit newData) {
		...
		if (obs == rawDataBlock) {
			...
			chan = PamUtils.getChannelPos(chan, clickControl.clickParameters.channelBitmap);
			double maxAmp = 0.;
	...
	class LevelMeters extends PamPanel {
		...
		private void createMeters() {
			int nChan = PamUtils.getNumChannels(clickControl.clickParameters.channelBitmap);
...
//						PamColors.PamColor.BORDER);
				...
				channel = PamUtils.getNthChannel(i, clickControl.clickParameters.channelBitmap);
				subPanel.add(BorderLayout.NORTH, new JLabel(" " + channel));
