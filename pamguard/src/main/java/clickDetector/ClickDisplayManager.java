...
//			System.out.println(c.getName());
	...
	class ClickDisplayInfo implements ComponentListener, ContainerListener, InternalFrameListener, Serializable {
		...
		public void createUnit() {
			...
			/*
			 * explicitly change the icon size for linux - this didnt work
			 */
...
////			BufferedImage bi = new BufferedImage(img.getWidth(null), img.getHeight(null), BufferedImage.TYPE_INT_ARGB_PRE);
//			
////			Then blit the icon image to the buffered image, and resize it as you do so:
//			Graphics g = bi.createGraphics();
////			g.drawImage(img, 0, 0, pf.getFrameIcon().getIconWidth(), pf.getFrameIcon().getIconHeight(), null);
//			
...
//			newClickDisplay.getFrame().setFrameIcon(newIcon);
...
//			System.out.println(e.getMessage());
	...
	ClickBTDisplay findFirstBTDisplay() {
		for (int i = 0; i < windowList.size(); i++) {
			if (windowList.get(i).getClass() == ClickBTDisplay.class) {
