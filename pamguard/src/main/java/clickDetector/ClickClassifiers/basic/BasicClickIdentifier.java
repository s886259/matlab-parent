...
public class BasicClickIdentifier implements ClickIdentifier, PamSettings {
	...
	BasicClickIdParameters idParameters = new BasicClickIdParameters();
...
//				clickDetector.newParameters();
	...
	public PamSymbol getSymbol(ClickDetection click) {
		...
		for (int i = 0; i < idParameters.clickTypeParams.size(); i++) {
			if (click.getClickType() == idParameters.clickTypeParams.get(i).code) {
	...
	public String[] getSpeciesList() {
		...
		for (int i = 0; i < idParameters.clickTypeParams.size(); i++) {
			strings[i] = idParameters.clickTypeParams.get(i).name;
		}
		return strings;
	}
	...
	public int codeToListIndex(int code) {
		if (codeToIndex == null || code >= codeToIndex.length) {
			int maxCode = code;
			for (int i = 0; i < idParameters.clickTypeParams.size(); i++) {
				maxCode = Math.max(maxCode, idParameters.clickTypeParams.get(i).code);
			}
			codeToIndex = new int[maxCode+1];
			for (int i = 0; i < idParameters.clickTypeParams.size(); i++) {
				codeToIndex[idParameters.clickTypeParams.get(i).code] = i;
			}
		}
    ...
    public int[] getCodeList() {
        ...
        for (int i=0; i < idParameters.clickTypeParams.size(); i++) {
            clickList[i]=idParameters.clickTypeParams.get(i).code;
        }
        return clickList;
    }
