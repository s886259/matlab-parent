...
public class SweepClassifierDialog extends PamDialog {
	...
	private class OptionsBlock extends IdBlock implements FFTLengthModeled, CodeHost {
		...
		OptionsBlock() {
			...
			c.gridx++;
			c.gridwidth = 1;
			addComponent(p, new JLabel(" around the click centre"), c);
		...
		protected void setParams() {
			if (sweepClassifierSet == null) {
				...
				name.setText(sweepClassifierSet.name);
				setCode(sweepClassifierSet.speciesCode);
			}
	...
	private class LengthBlock extends IdBlock {
		...
		LengthBlock() {
			...
			c.gridx++;
			addComponent(p, minLength = new JTextField(3), c);
			c.gridx++;
			addComponent(p, new JLabel(" to "), c);
			c.gridx++;
			addComponent(p, maxLength = new JTextField(3), c);
			c.gridx++;
	...
	class ZeroXBlock extends IdBlock {
		...
		@Override
		protected boolean getParams() {
			if ((sweepClassifierSet.enableZeroCrossings = getEnableBox()) == false) {
				return true;
			}
