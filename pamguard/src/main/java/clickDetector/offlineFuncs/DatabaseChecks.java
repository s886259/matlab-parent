...
public class DatabaseChecks {
	...
	private void checkEvents(boolean repair) {
		databaseCheckObserver.checkOutputText("First check event times and click counts ...", 0);
	...
	private boolean checkEvent(int eventId, boolean repair) {
		...
		try {
			...
			while (result.next()) {
				nClicks++;
				clickLogging.transferDataFromResult(result);			
				ts = (Timestamp) clickLogging.getTableDefinition().getTimeStampItem().getValue();
				resultTime = PamCalendar.millisFromTimeStamp(ts);
		...
		if (nClicks > 0) {
			// can only update the times if there are some clicks, otherwise
			// the times get set to Long.MIN_VALUE and Long.MAX_VALUE which really
			// screws the database.
			long diffTime = Math.abs(anEvent.getTimeMilliseconds() - firstTime);
			if (diffTime > 1000) {
			...
			diffTime = Math.abs(anEvent.getEventEndTime() - lastTime);
			if (diffTime > 1000) {
		...
		if (updated) {
			...
			databaseCheckObserver.checkOutputText(String.format("Event %d ok", anEvent.getDatabaseIndex()), 0);
		}
	...
	private void checkClicks(boolean repair) {
		databaseCheckObserver.checkOutputText("Now check that all database clicks match an event ...", 0);
