...
public class ClickTrainGraphics extends PamDetectionOverlayGraphics {
	...
	ClickTrainGraphics(ClickControl clickControl, ClickGroupDataBlock<ClickTrainDetection> clickTrains) {
		super(clickTrains);
//		super(clickControl.clickDetector, 
//				NewClickOverlayGraphics.SHOW_MAP | NewClickOverlayGraphics.SHOW_SPECTROGRAM | NewClickOverlayGraphics.SHOW_RADAR,
//				"Click Trains");
//		getPamSymbol().setSymbol(PamSymbol.SYMBOL_DIAMOND);
		setPamSymbol(new PamSymbol(PamSymbol.SYMBOL_DIAMOND, 15, 15, true, Color.RED, Color.RED));
	...
	private boolean shouldPlot(ClickTrainDetection clickTrainDetection) {
		if (clickTrainDetection.getNumLatLong() > 0) {
			return true;
		}
