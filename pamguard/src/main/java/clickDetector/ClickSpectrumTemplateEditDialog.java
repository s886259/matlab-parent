...
public class ClickSpectrumTemplateEditDialog extends PamDialog{
	...
	public ClickSpectrumTemplateEditDialog(Window parentFrame, Point pt,
		...
		westPanel.setBorder(new TitledBorder("Template Preview"));
		westPanel.add(BorderLayout.SOUTH, controls);
	...
	private void createEventsAverage(){
		...
		fftAll=getEventFFTs(channel,true);
	...
	public ArrayList<double[]>  getEventFFTs(Integer channel, boolean normalise ){
		...
		for (int i=0; i<selectedEvents.size(); i++){
			for (int j=0; j<selectedEvents.get(i).getSubDetectionsCount(); j++){
				 click=selectedEvents.get(i).getSubDetection(j);
				
				channels=PamUtils.getChannelArray (click.getChannelBitmap());
				
//				System.out.println("channel: "+channel);
//				System.out.println("");
//				for (int w=0; w<channels.length; w++){
//					System.out.print("ch: "+channels[w]);
//				}
//				System.out.println("");
...
//						 	printFFT(fft);
	...
	private void printFFT(double[] fft){
	...
	/**
	 * Class which create a table of all templates shown in the click spectrum. Tables can be deleted from memory or hidden from view in this menu. Allows for more streamlined management of multiple templates.
	 * @author EcologicUK
	 *
	 */
	class TemplateTable extends JPanel{
		...
		/**
		 * This will only be called if a table property is changed- not just a row change
		 * @author spn1
		 *
		 */
	...
	/**
	 * Creates the plot panel and axis of the template preview window. Shows a mean spectrum of the selected event/s along with confidence intervals. The tamplate can be viewed with a normal or log axis. 
	 * 
	 * @author EcologicUK
	 *
	 */
	 class SpectrumPlotPanel   {
		...
		private SpectrumPlotPanel(){
			...
			plotPanel.setPreferredSize(new Dimension(200, 400));
