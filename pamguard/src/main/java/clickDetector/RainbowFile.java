...
public class RainbowFile implements ClickFileStorage {
	...
	public boolean openClickStorage(File oldFile) {
		...
		ClickDetection click = new ClickDetection();
		boolean ans = false;
		long filePos;
			...
			while(true) {
				...
				while (true) {
					...
					if (click.dataType == HEADER_CLICK) {
						...
						file.skipBytes((int) (click.getDuration()*4));
					}
					else {
	...
	synchronized public boolean writeClick(ClickDetection click) {
		...
		try {
			...
			for (int iS = 0; iS < click.getDuration(); iS++) {
	...
	synchronized private boolean writeClickHeader(WriteWinFile winFile, ClickDetection click) {
		...
		try {
			...
			winFile.writeWinShort((int) click.getDuration());
			...
			if (click.flags == 0) {
				...
				if (click.getClickLocalisation() != null) {
					delay = click.getClickLocalisation().getFirstDelay();
					delayCorrection = click.getClickLocalisation().getDelayCorrection();
				}
	...
	synchronized private boolean readClickHeader(ClickDetection click) {
		...
		try {
			...
			click.setMeasuredAmplitudeType(AcousticDataUnit.AMPLITUDE_SCALE_LINREFSD);
	...
	private boolean readWavData(ClickDetection click) {
		...
		int duration = (int) click.getDuration();
	...
	private boolean skipWavData(ClickDetection click) {
		int nChan = PamUtils.getNumChannels(click.getChannelBitmap());
		int dataBytes = (int) (nChan * 2 * click.getDuration());
		try {
...
class ANALTYPE
{
  public:
    ...
    bool checkMultiChannels(int nChannels, bool autoFix);
		 ...
		 */
		int channels = clickDetector.getClickControl().clickParameters.channelBitmap;
