...
public class NewClickOverlayGraphics extends PamDetectionOverlayGraphics {
	...
	public NewClickOverlayGraphics(ClickControl clickControl, PamDataBlock parentDataBlock, int drawTypes, String name) {
		...
		PamSymbolManager.getInstance().removeManagedSymbol(this);
		this.setPamSymbol(new PamSymbol(PamSymbol.SYMBOL_CIRCLE, 10, 10, true, Color.BLACK, Color.BLACK));
...
//	}
	...
	protected Component makeMultiColourIcon(int iconStyle, PamSymbol forceSymbolType) {
		...
		for (int whaleId = 1; whaleId <= nRows * nCols; whaleId++) {
			pamSymbol = ClickBTDisplay.getClickSymbol(whaleId).clone();
...
//		}
	...
	protected PamKeyItem createRadarKey(int keyType) {
		if (keyType == PamKeyItem.KEY_SHORT) {
			return new BasicKeyItem(ClickBTDisplay.getClickSymbol(0).clone(), name);
		}
		else {
...
//		return super.drawOnSpectrogram(g, pamDataUnit, generalProjector);
		...
		if (click.eventId != 0) {
			Color col = PamColors.getInstance().getWhaleColor(click.eventId);
		...
		specSymbol.setSymbol(PamSymbol.SYMBOL_TRIANGLEU);
		specSymbol.draw(g, pt);
		specSymbol.setSymbol(PamSymbol.SYMBOL_TRIANGLED);
		pt.y = specSymbolSize/2;
		specSymbol.draw(g, pt);
	...
	private PamSymbol specSymbol = new PamSymbol(PamSymbol.SYMBOL_TRIANGLEU, specSymbolSize, specSymbolSize, true, Color.RED, Color.RED);
...
//		}
	...
	private boolean shouldDrawOnMap(ClickDetection cd) {
		return (cd.getClickType() > 0 || cd.getEventId() > 0);
	}
