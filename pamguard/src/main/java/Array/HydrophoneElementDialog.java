...
//		this.setAlwaysOnTop(true);
		setHelpPoint("docs.Array_NewHydrophone");
	}
	...
	static public Hydrophone showDialog(Window window, Hydrophone oldhydrophone, 
		...
		newHydrophone = isNew;
		if (isNew) {
			singleInstance.setTitle("New hydrophone");
		}
		else {
			singleInstance.setTitle("Edit hydrophone");
		}
...
//			constraints.insets = new Insets(2,2,2,2);
			constraints.gridx = 0;
			constraints.gridy = 0;
			addComponent(this, new JLabel("Hydrophone id ", SwingConstants.RIGHT), constraints);
			constraints.gridx++;
...
//			constraints.anchor = GridBagConstraints.;
			...
			constraints.gridx = 0;
			constraints.gridy++;
			addComponent(this, new JLabel("Hydrophone type ", SwingConstants.RIGHT), constraints);
			constraints.gridx++;
...
//			constraints.insets = new Insets(2,2,2,2);
			...
			constraints.gridx++;
			addComponent(this, new JLabel(" m (relative to streamer)"), constraints);
			...
			constraints.gridx++;
			addComponent(this, new JLabel(" m (distance ahead of the vessel)"), constraints);

			constraints.gridx = 0;
			constraints.gridy ++;
			constraints.anchor = GridBagConstraints.EAST;
			addComponent(this, new JLabel("depth ", SwingConstants.RIGHT), constraints);
			constraints.gridx++;
			constraints.anchor = GridBagConstraints.WEST;
			...
			constraints.gridx++;
			addComponent(this, new JLabel(" m (depth is positive)"), constraints);
		...
		void setParams() {
			if (newHydrophone == false) {
				...
				depth.setText(formatDouble(hydrophone.getDepth()));
		...
		boolean getParams() {
			try {
				...
				hydrophone.setDepth(Double.valueOf(depth.getText()));
	...
	class SensitivityPanel extends JPanel {
		...
		JTextField hSens, preampGain, bandwidth0, bandwidth1;
		...
		void setParams() {
			hSens.setText(String.format("%.1f", hydrophone.getSensitivity()));
		...
		boolean getParams() {
			try {
				hydrophone.setSensitivity(Double.valueOf(hSens.getText()));
				...
				JOptionPane.showMessageDialog(this, "There is a problem with one of the parameters in the Sensitivity panel");
				return false;
			}
			return true;
		}
