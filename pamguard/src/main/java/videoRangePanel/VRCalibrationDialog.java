...
//		}
		...
		else if (vrControl.vrParameters.getCurrentCalibrationData() != null) {
			singleInstance.vrCalibrationData = vrControl.vrParameters.getCurrentCalibrationData().clone();
		}
	...
	void enableButtons() {
		if (replaceButton != null) {
			replaceButton.setEnabled(vrControl.vrParameters.getCurrentCalibrationData() != null);
		}
	}
	...
	void updateCalibration() {
		...
		Point p1, p2;
		p1 = vrControl.getCalibratePoint1();
		p2 = vrControl.getCalibratePoint2();
		if (p1 == null || p2 == null) {
			return;
		}
...
//		double ang = Math.atan(vrCalibrationData.objectSize_cm / 2 / 100 / vrCalibrationData.objectDistance_m) * 2;
	...
	class AddButton implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if (getParams()) {
				vrControl.vrParameters.addCalibrationData(vrCalibrationData);
				setVisible(false);
			}
		}
	}
	class ReplaceButton implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if (getParams() && vrControl.vrParameters.getCurrentCalibrationData() != null) {
				vrControl.vrParameters.getCurrentCalibrationData().update(vrCalibrationData);
				vrCalibrationData = vrControl.vrParameters.getCurrentCalibrationData();
				setVisible(false);
			}
		}
	}
