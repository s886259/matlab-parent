...
public class AcceptMeasurementDialog extends PamDialog {
	...
	public void setParams() {
		...
		range.setText(String.format("%.1f m", vrMeasurement.distanceMeters));
		accuracy.setText(String.format("± %.1f m/pix", vrMeasurement.pixelAccuracy));
		
		if (vrMeasurement.cameraAngle != null) {
			cameraAngle.setText(String.format("%.1f", vrMeasurement.cameraAngle));
...
//		vrMeasurement.comment = comment.getText();
		...
		try {
			...
			vrMeasurement.totalAngle = Double.valueOf(angleTotal.getText());
		}
		catch (NumberFormatException e) {
			return false;
		}
		try {
			vrMeasurement.cameraAngle = Double.valueOf(cameraAngle.getText());
		}
		catch (NumberFormatException e) {
			vrMeasurement.cameraAngle = null;
		}
		return true;
	}
