...
//	FFTDataSource parentFFTProcess;
	...
	public Annotation getAnnotation(PamDataBlock pamDataBlock, int annotation) {
		...
		return new Annotation(this, this, "Noise Reduction", "Gaussian Kernel Smoothing");
	}
...
//				arg.getStartSample(), arg.getDuration(), arg.getChannelBitmap());
		...
		private void smoothData(int dataLength, Complex[] newData) {
			...
			for (int iF = 0; iF < dataLength; iF++) {
				...
				for (dF = 0; dF < kernelSize; dF++) {
					...
					if (tF < dataLength && tF >= 0) {
						for (dT = 0; dT < kernelSize; dT++) {
							...
							realBodge += (localList[dT][tF].magsq() * kernel[dT][dF]);
						}
					}
				}
				/*
				 ...
				 */
				mag = localList[1][iF].mag();
				phaseX = localList[1][iF].real / mag;
				phaseY = localList[1][iF].imag / mag;
				mag = Math.sqrt(realBodge);
				newData[iF].assign(mag * phaseX, mag * phaseY);
			}
