...
public class StorageOptionsDialog extends PamDialog {
	...
	private void createControls() {
		...
		BinaryStore binaryStore = (BinaryStore) pamController.findControlledUnit(BinaryStore.unitType);
...
//			cb.setVisible(hasBinaryStore);
			...
			if (hasBinaryStore) {
				cb.setSelected(storageParameters.isStoreBinary(aDataBlock.getDataName(), true));
			}
			bsCheckBoxes.add(cb);
...
//			cb.setVisible(hasDatabase);
			...
			if (hasDatabase) {
				cb.setSelected(storageParameters.isStoreDatabase(aDataBlock.getDataName(), !hasBinaryStore));
			}
			dbCheckBoxes.add(cb);
		}
	...
	public boolean getParams() {
		...
		for (int i = 0; i < usedDataBlocks.size(); i++) {
			...
			storageParameters.setStorageOptions(usedDataBlocks.get(i).getDataName(), storeDatabase, storeBinary);
		}
		if (errors > 0) {
