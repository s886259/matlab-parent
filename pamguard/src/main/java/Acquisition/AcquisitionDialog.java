...
public class AcquisitionDialog extends PamDialog {
	...
	private JTextField preampGain, bandwidth0, bandwidth1;
...
//	private JComboBox panelChannelList[] = new JComboBox[PamConstants.MAX_CHANNELS];
	...
	private AcquisitionDialog (Frame parentFrame) {
		...
		if (PamController.getInstance().getRunMode() == PamController.RUN_PAMVIEW) {
			...
			offlineDAQDialogPanel = new OfflineDAQDialogPanel(acquisitionControl, this);
	...
	public boolean getParams() {
		...
		if (currentDaqSystem != null){
			return currentDaqSystem.dialogGetParams();
		}
...
//		constraints.gridy++;
	...
	// useful func for gridbaglayour use
//	private void addComponent(JPanel panel, Component p, GridBagConstraints constraints){
...
//	}
	 ...
	 * 
	 */
	int getNumChannels() { //Xiao Yan Deng
		try {
			/*
			if(Integer.valueOf(nChannels.getText())>0){
			return Math.min(Integer.valueOf(nChannels.getText()), panelChannelList.length);
			}
			else{
				return panelChannelList.length;	
			}*/
			return Integer.valueOf(nChannels.getText());
			//return acquisitionParameters.nChannels;
		}
		catch (Exception Ex) {
			return 0;
		}
