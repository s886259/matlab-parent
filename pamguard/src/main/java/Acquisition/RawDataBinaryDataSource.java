...
public class RawDataBinaryDataSource extends BinaryDataSource {
	...
	public PackedBinaryObject getPackedData(PamDataUnit pamDataUnit) {
		...
		int duration = (int) rawDataUnit.getDuration();
		int nBytes = 2;
		...
		PackedBinaryObject bo = new PackedBinaryObject(0, bos.toByteArray());
		return bo;
	}
...
//					binaryObjectData.getDataLength(), duration, nBytes));
		...
		RawDataUnit rawDataUnit = new RawDataUnit(binaryObjectData.getTimeMillis(), channelMap, startSample, duration);
