...
public class DbHtDisplayPanel {
	...
	DbHtDisplayPanel(DbHtControl dbHtControl, DbHtTabPanel dbHtTabPanel) {
		...
		hScroller = new PamScroller("dBHt Scroller", AbstractPamScroller.HORIZONTAL, 1000, 600000, isViewer);
	...
	public void timeRangeChanged(double newValue) {
		...
		hScroller.setVisibleAmount((long) (newValue*1000));
		repaintAll();
	...
	private class PlotPanel extends JPanelWithPamKey {
		...
		public PlotPanel() {
			super();
			measureSymbols[0] = new PamSymbol(PamSymbol.SYMBOL_CIRCLE, 10, 10, true, defaultColours[0], Color.BLUE);
			measureSymbols[1] = new PamSymbol(PamSymbol.SYMBOL_DIAMOND, 10, 10, true, defaultColours[1], Color.GREEN);
			measureSymbols[2] = new PamSymbol(PamSymbol.SYMBOL_STAR, 10, 10, true, defaultColours[2], Color.RED);
			createKey();
		...
		protected void paintComponent(Graphics g) {
			...
			boolean first = true;
			Color col;
			synchronized (measurementDataBlock) {
