...
public class DbHtControl extends PamControlledUnit implements PamSettings {
	...
	protected boolean fillXMLParameters(Document doc, Element paramsEl) {
		paramsEl.setAttribute("channelMap", new Integer(dbHtParameters.channelMap).toString());
		paramsEl.setAttribute("measurementInterval", new Integer(dbHtParameters.measurementInterval).toString());
		paramsEl.setAttribute("filterLogOrder", new Integer(dbHtParameters.filterLogOrder).toString());
		paramsEl.setAttribute("chebyGamma", new Double(dbHtParameters.chebyGamma).toString());
		paramsEl.setAttribute("numHtPoints", new Double(dbHtParameters.getNumHtPoints()).toString());
		int n = dbHtParameters.getNumHtPoints();
		Element htEl;
		for (int i = 0; i < n; i++) {
			htEl = doc.createElement("HTPoint");
			htEl.setAttribute("frequency", new Double(dbHtParameters.getFrequencyPoints()[i]).toString());
			htEl.setAttribute("hearingThreshold", new Double(dbHtParameters.hearingThreshold[i]).toString());
			paramsEl.appendChild(htEl);
		}
