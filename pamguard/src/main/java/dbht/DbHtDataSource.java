...
public class DbHtDataSource extends BinaryDataSource {
	...
	public PackedBinaryObject getPackedData(PamDataUnit pamDataUnit) {
		 ...
		 */
		try {
			dos.writeLong(du.getStartSample());
			dos.writeInt(du.getChannelBitmap());
...
//		getBinaryStorageStream().storeData(1, du.getTimeMilliseconds(), bos.toByteArray());
	...
	public PamDataUnit sinkData(BinaryObjectData binaryObjectData,
		...
		DbHtDataUnit du = new DbHtDataUnit(binaryObjectData.getTimeMillis(), channelMap, startSample, duration);
		du.setRms(rms/100.);
