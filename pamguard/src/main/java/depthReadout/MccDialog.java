...
//		}
	...
	class SensorPanel extends JPanel {
		...
		public SensorPanel(int iSensor) {
			...
			c.gridx = 0;
			c.anchor = GridBagConstraints.WEST;
			addComponent(this, new JLabel("Hydrophones"), c);
...
//			hPanel.setBorder(new TitledBorder(""));
			...
			hydrophones = new JCheckBox[nH];
			for (int i = 0; i < nH; i++) {
				c2.gridy = 0;
				addComponent(hPanel, new JLabel(String.format("%d", i)), c2);	
				hydrophones[i] = new JCheckBox();
				c2.gridy = 1;
				addComponent(hPanel, hydrophones[i], c2);
				c2.gridx++;
			}
...
//			c.gridy++;
			...
			if (hydrophones != null) for (int i = 0; i < hydrophones.length; i++) {
				hydrophones[i].setToolTipText("Select which hydrophones use data from this sensor");
			}
		}
		...
		void setParams() {
			...
			if (depthParameters.hydrophoneMaps != null && depthParameters.hydrophoneMaps.length > iSensor) {
				for (int i = 0; i < hydrophones.length; i++) {
					hydrophones[i].setSelected((depthParameters.hydrophoneMaps[iSensor] & 1<<i) != 0);
				}
			}
	...
	class BoardPanel extends JPanel {
		...
		public BoardPanel() {
			...
			int[] ranges = MccJniInterface.bipolarRanges;
			for (int i = 0; i < ranges.length; i++) {
				rangeList.addItem(MccJniInterface.sayBibolarRange(ranges[i]));
			}
...
//			nSensors.
		...
		void setParams() {
			...
			int nBoards = mccJniInterface.getNumBoards();
			for (int i = 0; i < nBoards; i++) {
				boardList.addItem(mccJniInterface.getBoardName(i));
			}
			if (mccDepthParameters.iBoard >= 0 &&
					mccDepthParameters.iBoard < nBoards) {
			...
			int ind = MccJniInterface.getBipolarRangeIndex(mccDepthParameters.range);
			if (ind >= 0) {
		...
		boolean getParams() {
			...
			mccDepthParameters.range = MccJniInterface.bipolarRanges[rangeList.getSelectedIndex()];
