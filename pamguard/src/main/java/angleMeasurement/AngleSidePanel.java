...
public class AngleSidePanel implements PamSidePanel {
	...
	public void newAngle(AngleDataUnit angleDataUnit) {
		currentAngle.setText(String.format("Corrected angle = %.1f\u00B0", angleDataUnit.correctedAngle));
		if (first) {
	...
	protected void showHeldAngle(AngleDataUnit angleDataUnit) {
		if (angleDataUnit != null){
			heldAngle.setText(String.format("Held: %.1f\u00B0", angleDataUnit.correctedAngle));
		}
		else {
			heldAngle.setText(" ");
		}
