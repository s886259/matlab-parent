	...
	public void newData(PamObservable o, PamDataUnit arg) {
		...
		if (newData != null) {
			...
			newData = new RawDataUnit(rawData.getTimeMilliseconds(), rawData.getChannelBitmap(), 
					rawData.getStartSample(), rawData.getDuration());
		}
		outputData = newData.getRawData();
		inputData = rawData.getRawData();
		if (outputData == null ||outputData.length != rawData.getDuration()) {
			outputData = new double[(int) rawData.getDuration()];
		}
		
		for (int i = 0; i < rawData.getDuration(); i++) {
			outputData[i] = inputData[i] * gain;
		}
		
//		newData.amplifyMeasuredAmplitudeByLinear(gain);
		newData.setMeasuredAmplitude(rawData.getMeasuredAmplitude() * gain);
