package com.tp.matlab.web.frequency.envolope.controller;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.tp.matlab.web.frequency.envolope.param.FreqEnvolopeResponse;
...
@Slf4j
@RestController
@RequestMapping(value = "/api/v1/freq/envolope/")
@RequiredArgsConstructor
@Api(value = "包络频谱图", tags = "包络频谱图")
public class FreqEnvolopeController {

    private final FreqEnvolopeService freqEnvolopeService;

    @ApiOperation(value = "从数组中生成包络频谱图")
    @PostMapping("fromArray")
    public ResponseEntity<FreqEnvolopeResponse> fromArray(
            @RequestBody @Valid FromArrayRequest fromArrayRequest
    ) throws JsonProcessingException {
        return execute(fromArrayRequest.getArray(), null, null);
    }

    @ApiOperation(value = "上传文件生成包络频谱图")
    @ApiImplicitParams({
    ...
    public ResponseEntity<FreqEnvolopeResponse> fromUpload(
    ...
    private ResponseEntity<FreqEnvolopeResponse> execute(
            @NonNull final List<Double> records,
            @Nullable Integer columnIndex,
            @Nullable String file
    ) throws JsonProcessingException {
        final FreqEnvolopeResponse response = toValue(freqEnvolopeService.execute(records), FreqEnvolopeResponse.class);
