package com.tp.matlab.web.frequency.envolope.param;
...
@NoArgsConstructor
public class FreqEnvolopeResponse {
    ...
    @ApiModelProperty(value = "真峰峰值", required = true)
    ...
    @ApiModelProperty(value = "振动总值，gE（用于计算整体趋势）", required = true)
    private BigDecimal tv;

    @ApiModelProperty(value = "x轴,时间/s", required = true)
    private List<BigDecimal> x;

    @ApiModelProperty(value = "y轴,加速度b包络时域/ gE", required = true)
