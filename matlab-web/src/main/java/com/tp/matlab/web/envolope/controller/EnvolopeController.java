package com.tp.matlab.web.envolope.controller;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.tp.matlab.web.envolope.param.EnvolopeResponse;
import com.tp.matlab.web.envolope.service.EnvolopeService;
...
@Slf4j
@RestController
@RequestMapping(value = "/api/v1/envolope/")
@RequiredArgsConstructor
@Api(value = "包络时域图", tags = "包络时域图")
public class EnvolopeController {

    private final EnvolopeService envolopeService;

    @ApiOperation(value = "从数组中生成包络时域图")
    @PostMapping("fromArray")
    public ResponseEntity<EnvolopeResponse> fromArray(
    ...
    public ResponseEntity<EnvolopeResponse> fromUpload(
    ...
    @PostMapping("fromUrl")
    public ResponseEntity<EnvolopeResponse> fromUrl(
    ...
    private ResponseEntity<EnvolopeResponse> execute(
            @NonNull final List<Double> records,
            @Nullable Integer columnIndex,
            @Nullable String file
    ) throws JsonProcessingException {
        final EnvolopeResponse response = toValue(envolopeService.execute(records), EnvolopeResponse.class);
